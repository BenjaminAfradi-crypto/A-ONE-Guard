(async()=>{
 ...
 form.onsubmit=async e=>{e.preventDefault();err.textContent='';const f=new FormData(form);AONE.loading(true,'Firmenkonto wird angelegt…');try{
   ...
   localStorage.setItem('aone_guard_pending_company',JSON.stringify({email,company,name,created_at:Date.now()}));
   ...
   await AONE.rpc('guard_create_organization',{p_name:company}); await AONE.chooseContext(); localStorage.removeItem('aone_guard_pending_company');
